function [df] = evaluate_green_function_partial_xinormal(x,xi,K,e,d,n)
%EVALUATE_GREEN_FUNCTION_NORMAL_DERIVATIVE Summary of this function goes here
%   Detailed explanation goes here
    grad = zeros(3,1);
    f = evaluate_green_function(x,xi,K,e,d);
    e2 = e^2;
    grad(1) = evaluate_green_function_partial_xi(x,xi,K,e,d,1,e2,f);
    grad(2) = evaluate_green_function_partial_xi(x,xi,K,e,d,2,e2,f);
    grad(3) = evaluate_green_function_partial_xi(x,xi,K,e,d,3,e2,f);
    df = dot(grad, n);
end

